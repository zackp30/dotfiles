...
* Org
  ...
  #+BEGIN_SRC emacs-lisp :tangle yes
    (use-package org
      ...
      (setq org-agenda-files `(,(expand-file-name "~/org/Events.org")))
      (define-key global-map (kbd "C-c l") 'org-store-link)
      (define-key global-map (kbd "C-c a") 'org-agenda)
      (setq org-log-done t
            org-latex-listings 'minted ;; enable minted syntax highlighting for LaTeX export
            org-latex-minted-options '(("frame" "lines")
                                       ("fontsize" "\\scriptsize")
                                       ("linenos" ""))
            org-latex-minted-langs '((conf-space "aconf")
                                     (conf-unix "squid"))
            org-latex-to-pdf-process '("xelatex -shell-escape -interaction nonstopmode -output-directory %o %f"
                                       "xelatex -shell-escape -interaction nonstopmode -output-directory %o %f"
                                       "xelatex -shell-escape -interaction nonstopmode -output-directory %o %f")
            org-twbs-head-extra (with-temp-buffer
                                  (insert-file-contents (expand-file-name "~/.homesick/repos/dotfiles/css.html"))
                                  (buffer-string)))
      (defun my-org-confirm-babel-evaluate (lang body)
        (not (string= lang "dot")))  ; don't ask to run a Graphviz block
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package mediawiki
    ...
    (add-to-list 'mediawiki-site-alist
                 '("Wikipedia" "https://en.wikipedia.org/" "zackp30" "REDACTED" "Main page")))
#+END_SRC
