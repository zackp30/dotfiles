...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package helm-swoop
    :config
    (define-key isearch-mode-map (kbd "M-a") 'helm-swoop-from-isearch))
#+END_SRC
...
Disabled because of an (apparent) bug in Emacs.
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package smex
    :bind ("M-x" . smex)
    :bind ("M-x" . execute-extended-command) ;; in case smex breaks
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (load-theme 'material t) ;; Yay material design.
