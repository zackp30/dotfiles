   ...
   #+BEGIN_SRC emacs-lisp :tangle yes
     ...
     (defun z/get-quote ()
       (let* ((quote-json (json-read-file "~/.quotes.json"))
              (quote-text (cdr (assoc 'text (elt quote-json (random (length quote-json))))))
              (quote-author (cdr (assoc 'author (elt quote-json (random (length quote-json)))))))
         (list quote-text quote-author)))

     (defun z/mu4e-sig ()
       (let ((-quote (z/get-quote)))
         (setq message-signature (concat "
      Zack Piper         PGP: 409B 6B17 96C2 7546 2A17  0311 3804 BB82 D39D C0E3
