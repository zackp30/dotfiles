...
** =org-agenda-files=
   ...
   #+BEGIN_SRC emacs-lisp :tangle yes
     ...
     (setq org-agenda-files (append (find-lisp-find-files "~/org/" "\.org$") (find-lisp-find-files "~/MassRevision" "\.org$")))
   #+END_SRC
** =org-log-done=
...
** Org-capture + Org-protocol
   ...
   Template declarations are in [[file:~/.emacs.d/init.el]] so I can use M-x customize-variable RET with it.
...
**** Link and text
     ...
     location.href='org-protocol://capture://L/'+encodeURIComponent(location.href)+'/'+encodeURIComponent(document.title)+'/'+encodeURIComponent(window.getSelection())
     #+END_SRC
