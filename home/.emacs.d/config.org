...
** =z-portfolio/date=
   #+BEGIN_SRC emacs-lisp :tangle yes
     (defun z-portfolio/date (date)
       (interactive "sDate: ")
       (mu4e-headers-search (concat "to:zack@infosecpartners.co.uk " "date:" date)))
   #+END_SRC
...
** Portfolio keymap
   ...
   #+BEGIN_SRC emacs-lisp :tangle yes
     ...
     (global-set-key (kbd "C-x s s") 'z-portfolio/date)
   #+END_SRC
...
+---+---+---+
   ...
   #+BEGIN_SRC emacs-lisp :tangle yes
     (defun z-sig/work () "Work signature"
            (string-join  (list"+----------------------------------------+"
                               "|◤                                      ◥|"
                               "| Zack Piper            Security Analyst |"
                               "|                       InfoSec Partners |"
                               "|                                        |"
                               "|      http://infosecpartners.com        |"
                               "|󠀳                                        |"
                               ...
                               "|◤                                      ◥|"
                               "|                                        |"
                               "| Mobile              (+44) 7460 193762  |"
                               "| Main line           (+44) 1256 893662  |"
                               "|                                        |"
                               "|◣                                      ◢|"
                               "+----------------------------------------+"
                               ""
                               ...
                               "             +---+---+---+"
                               ""
                               "PRIVILEGED AND CONFIDENTIAL: This communication, along with any documents,"
                               "files or attachments, is privileged, confidential or otherwise exempt from"
                               "disclosure. It is intended only for the use of the individual or entity"
                               "named above and others who have been specifically authorized to receive it."
                               ""
                               "If you are not the intended recipient, you are hereby notified that any"
                               "dissemination, distribution, use or copying of this communication or its"
                               "attachments is strictly prohibited. If you received this communication in"
                               "error, please notify us immediately. We advise you to carry out your own"
                               "virus check before opening any attachment, as we cannot accept liability"
                               "for any damage sustained as a result of any software viruses.
                               ""Thank you.")
                          "\n"))

     (defun z-sig/personal () "Personal signature"
            (string-join  (list "Zack Piper          System administrator"
                                "        https://apertron.net            ")
                          "\n"))


     (defun z/mail-sig () "Generate mail signature"
            (if (string= "Work" (mu4e-context-name (mu4e-context-current)))
                (z-sig/work)
              (z-sig/personal)))
     (setq mu4e-compose-signature 'z/mail-sig)
   #+END_SRC
...
** Graphviz
   ...
   #+BEGIN_SRC emacs-lisp :tangle yes
     (use-package graphviz-dot-mode
       :quelpa (graphviz-dot-mode :fetcher git :url "https://github.com/wynro/graphviz-dot-mode"))
   #+END_SRC
...
** Smart-Mode-Line
  ...
  I have yet to get around to making my own mode-line, but
  Smart-Mode-Line is great, so I don't see why I need to, other than
  for fun of course.
...
** Org directory
   ...
   =~/onotes= seems the best place to store such things.
  ...
  #+BEGIN_SRC emacs-lisp :tangle yes
    (use-package org
      ...
      (setq org-directory (expand-file-name "~/onotes/"))
  #+END_SRC
...
** =org-agenda-files=
   ...
   #+BEGIN_SRC emacs-lisp :tangle yes
     ...
     (setq org-agenda-files
           (append (find-lisp-find-files org-directory "\.org$")))
   #+END_SRC
** =org-log-done=
   ...
   #+BEGIN_SRC emacs-lisp :tangle yes
     (defun z/org-confirm-babel-evaluate (lang body)
       (not (or (string= lang "dot") ;; don't ask to run a Graphviz block
                ...
                (string= lang "shell") ;; don’t ask for gnuplot blocks
...
** org-gcal
   ...
   #+BEGIN_SRC emacs-lisp :tangle yes
     (define-key org-mode-map (kbd "C-c C-p") 'org-gcal-post-at-point)
   #+END_SRC
