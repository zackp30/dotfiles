...
* Introduction
  ...
  -  Some LISP variants
    - [[http://clojure.org][Clojure]] (50%) --- Java Virtual Machine based language which
      is a lot like LISP.
    - [[http://racket-lang.org][Racket]]  [2] (30%) --- an interesting language which is useful for
...
*** Details on my computing life
    ...
    keep an eye on the logs (=journalctl -fl=) under a tmux  [7] session and
...
**** My document setup
     ...
     I write all my schoolwork using Emacs in Markdown, using Pandoc  [8],
     ...
     my custom build rules written in Haskell  [9], and live reloaded by
     Guard  [10]. My Emacs sessions runs under a tmux  [11] session, which
     uses tmux-powerline  [12].

**** My miscellaneous setup

     -  Emails are, of course, written in Emacs.
     -  Programming done in, you guessed it: Emacs.
     -  =mutt=  [13] is my email client.
     -  =weechat=  [14] is my IRC client.
     -  My version control of choice is Git  [15].
     -  Linux distribution of choice is openSUSE  [16].
     -  Todo list manager of choice is  [17]
     -  Shell of choice is zsh  [18]
     -  Browser of choice is Chromium /or/ Elinks  [19] depending on if I'm
...
***** Story
      ...
      friend on IRC told me about Ruby, and a Ruby training site [20], Ruby
      ...
      [4] http://trac.sacrideo.us/wg/wiki/R7RSHomePage

      [5] http://haskell.org

      [7] http://tmux.github.io

      [8] http://pandoc.org

      [9] https://github.com/zackp30/basedocument-ng

      [10] http://guardgem.org/

      [11] http://tmux.github.io

      [12] http://powerline.readthedocs.org/en/master/

      [13] http://www.mutt.org/

      [14] http://weechat.org

      [15] http://git-scm.org

      [16] http:/opensuse.org

      [17] http://taskwarrior.org

      [18] http://www.zsh.org

      [19] http://elinks.or.cz/
